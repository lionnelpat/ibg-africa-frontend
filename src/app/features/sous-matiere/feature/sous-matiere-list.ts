...
import { SousMatiereApi } from '../data-access/sous-matiere.api';
...
@Component({
    selector: 'app-sous-matiere-list',
    ...
    template: `
        <p-card>
            ...
            <p-table
                ...
                [globalFilterFields]="['intitule', 'libelleLong', 'libelleCourt']"
                dataKey="id"
                [rowHover]="true"
        ...
        <p-dialog [(visible)]="dialogVisible" [style]="{ width: '640px' }" header="SousMatiere" [modal]="true">
            <ng-template #content>
...
export class SousMatiereList implements OnInit {
    private readonly api = inject(SousMatiereApi);
    private readonly notification = inject(NotificationService);
    private readonly fb = inject(NonNullableFormBuilder);

    rows = signal<SousMatiere[]>([]);
    ...
    openEdit(entity: SousMatiere): void {
    ...
    save(): void {
        ...
        const request$ = this.editingId ? this.api.update({ id: this.editingId, ...dto } as SousMatiere) : this.api.create(dto as Omit<SousMatiere, 'id'>);
    ...
    remove(entity: SousMatiere): void {
