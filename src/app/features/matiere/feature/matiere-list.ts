...
import { MatiereApi } from '../data-access/matiere.api';
...
@Component({
    ...
    template: `
        ...
        <p-dialog [(visible)]="dialogVisible" [style]="{ width: '640px' }" header="Matiere" [modal]="true">
            <ng-template #content>
...
export class MatiereList implements OnInit {
    private readonly api = inject(MatiereApi);
    ...
    save(): void {
        ...
        const request$ = this.editingId ? this.api.update({ id: this.editingId, ...dto } as Matiere) : this.api.create(dto as Omit<Matiere, 'id'>);
